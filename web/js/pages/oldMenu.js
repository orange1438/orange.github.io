...
var appendMenu = function(data) {
	...
	for (var i = 0; i < data.length; i++) {
		//内层排序
		data[i].menuInfoResultList = (data[i].menuInfoResultList).sort(function(a, b) {
			return a.sortNum - b.sortNum;
		});
	}
	...
	for (var i = 0; i < data.length; i++) {
		...
		dishesStr += "<div class='menu-dishes-classify' id='classify"+ data[i].categoryId +"'><div class='menu-dishes-classify-title'><p>"+ data[i].categoryName +"</p></div>";
		...
		for (var j = 0; j < dishes.length; j++) {
			...
			if (menuActivityInfo != null) {
				...
				MenuLabels = menuActivityInfo.MenuLabels;
			}
			//拼接时间
			...
			endTime += "00";
			//拼接菜品字符串
			dishesStr += "<div class='menu-food' foodId='"+ dishes[j].foodId +"' foodName='"+ dishes[j].foodName +"' foodPrice='"+ dishes[j].price +"' foodType='"+ dishes[j].foodType +"' discount='"+ food_discount +"' giveBeanCoins='"+ food_beans +"' MenuLabels='"+ MenuLabels +"' classify='"+ data[i].categoryId +"' categoryName='"+ data[i].categoryName +"' isCollect='"+ dishes[j].isFavorties +"' isQueHuo='"+ dishes[j].isQueHuo +"' sortNum='"+ dishes[j].sortNum +"' state='"+ dishes[j].state +"' startSupplyTime='"+ startTime +"' endSupplyTime='"+ endTime +"'>";
			//保存菜品
			foodArray.push(new food(dishes[j].foodId, dishes[j].foodName, dishes[j].price, foodNum, dishes[j].foodType, dishes[j].isFavorties, dishes[j].isQueHuo, food_discount, food_beans, data[i].categoryId, data[i].categoryName, MenuLabels, dishes[j].sortNum, dishes[j].state, startTime, endTime));
			//判断用户是否收藏该菜品
			if (dishes[j].isFavorties == 1) {
				collectNum++;
				dishesStr += "<p class='menu-food-name'><img src='img/icon-06.png' width='100%' /><span>"+ dishes[j].foodName +"</span></p>";
			} else {
				dishesStr += "<p class='menu-food-name'><img src='img/icon-07.png' width='100%' /><span>"+ dishes[j].foodName +"</span></p>";
			}
			//判断该菜品的优惠情况
			dishesStr += "<p class='menu-food-preferential'>";
			//判断是否有折扣
			if (food_discount != 100) {
				dishesStr += "<span>"+ (food_discount/100) +"</span>折&nbsp;&nbsp;";
			}
			//判断是否送豆币
			if (food_beans != 0) {
				dishesStr += "送<span>"+ food_beans +"</span>豆币";
			}
			dishesStr += "打6折</p>";
			dishesStr += "<p class='menu-food-price'>&yen;<span>"+ (dishes[j].price / 100) +"</span><small>/份</small></p>";
			dishesStr += "<p class='menu-food-labs'>";
			//循环该菜品的标签
			if (MenuLabels.length > 0) {
				for (var l = 0; l < MenuLabels.length; l++) {
					console.log(MenuLabels[l].name)
				}
			}
			dishesStr += "<p class='menu-food-btn'><b class='menu-food-btn-subtract hide'></b><span class='menu-food-btn-num hide'>0</span><b class='menu-food-btn-add'></b></p>";
			
			//判断是否不在贩卖时间
			var nowTime = parseInt($(".menu-dishes").attr("nowtime"));
			if (parseInt(startTime) > nowTime || parseInt(endTime) < nowTime) {
				//不再贩卖时间
				dishesStr += "<div class='menu-food-notTime'><img src='img/icon-34.png' /></div>";
			} else {
				dishesStr += "<div class='menu-food-notTime hide'><img src='img/icon-34.png' /></div>";
				//判断是否缺货
				if (parseInt(dishes[j].isQueHuo) == 1) {
					//缺货
					dishesStr += "<div class='menu-food-OOS'><img src='img/icon-35.png' /></div>";
				} else {
					dishesStr += "<div class='menu-food-OOS hide'><img src='img/icon-35.png' /></div>";
				}
			}
			dishesStr += "</div>";
		}
	...
	if (collectNum > 0) {
		...
		classifyStr = tempClassify + classifyStr;
	}
	$("#classify .swiper-slide").append(classifyStr);
	$("#dishes .swiper-slide").append(dishesStr);
...
var getClassifyAddress = function(offsetTop) {
	if ($(".classify-active").attr("data-categoryid") != "collectClassify") {
		var divOffTop = 0, lastDivOffTop = 0;
...
var reAppendFood = function(data, dataType) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (data[i].discount != 100) {
			str += "<span>" + (data[i].discount / 100) + "</span>折&nbsp;&nbsp;";
		}
		//判断是否送豆币
		if (data[i].beans != 0) {
			str += "送<span>" + data[i].beans + "</span>豆币";
		}
		str += "&nbsp;</p>";
		...
		if (data[i].MenuLabels.length > 0) {
			for (var l = 0; l < data[i].MenuLabels.length; l++) {
				console.log(data[i].MenuLabels[l].name)
			}
		}
		//判断是否选择了菜品
		if (data[i].foodNum != 0) {
...
//			classifySwiper.onResize();
		...
		case 0:
			...
			$(".menu-dishes").css("padding-bottom", "132px");
